...
class GraficoBoxplot(BaseGerador):
    ...
    def _filtrar_dados_com_tamanho_valido(self, df):
        logger = logging.getLogger('FileManager')
        logger.debug(f"BoxPlot - Filtrando dados com tamanho válido")
    ...
    def _criar_boxplot_operacoes(self, df):
        try:
            ...
            if df_com_tamanho.empty:
                logger.debug("BoxPlot - Operações: Sem dados válidos para plotar")
            ...
            if dados_boxplot:
                ...
                logger.debug("BoxPlot - Operações: Sem dados para boxplot após filtragem")
            ...
            logger = logging.getLogger('FileManager')
            logger.error(f"BoxPlot - Erro ao criar boxplot de operações: {str(e)}")
